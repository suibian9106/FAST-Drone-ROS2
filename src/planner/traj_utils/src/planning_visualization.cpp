...
PlanningVisualization::PlanningVisualization(const rclcpp::NodeOptions &options)
    : Node("planning_visualization", options) {
  // ROS2发布器创建方式
  goal_point_pub =
      this->create_publisher<visualization_msgs::msg::Marker>("goal_point", 2);
  global_list_pub =
      this->create_publisher<visualization_msgs::msg::Marker>("global_list", 2);
  init_list_pub =
      this->create_publisher<visualization_msgs::msg::Marker>("init_list", 2);
  optimal_list_pub = this->create_publisher<visualization_msgs::msg::Marker>(
      "optimal_list", 2);
  a_star_list_pub = this->create_publisher<visualization_msgs::msg::Marker>(
      "a_star_list", 20);
}
...
void PlanningVisualization::displayMarkerList(
  ...
  sphere.header.stamp = line_strip.header.stamp = this->now(); // ROS2时间获取
...
void PlanningVisualization::generateArrowDisplayArray(
  ...
  arrow.header.stamp = this->now(); // ROS2时间获取
...
void PlanningVisualization::displayGoalPoint(Eigen::Vector3d goal_point,
  ...
  sphere.header.stamp = this->now(); // ROS2时间获取
...
void PlanningVisualization::displayMultiInitPathList(
  ...
  for (int id = 0; id < last_nums; id++) {
    ...
    rclcpp::sleep_for(std::chrono::microseconds(1000)); // ROS2休眠方式
  }
  last_nums = 0;
...
void PlanningVisualization::displayAStarList(
    std::vector<std::vector<Eigen::Vector3d>> a_star_paths, int id) {
