#ifndef PLAN_CONTAINER_H_
#define PLAN_CONTAINER_H_
...
public:
  ...
  rclcpp::Time global_start_time_; // ROS2时间类型
  ...
  void setGlobalTraj(const PolynomialTraj &traj,
                     const rclcpp::Time &time) // 参数类型改为rclcpp::Time
  {
    global_traj_ = traj;
    global_traj_.init();
  ...
  void getTrajByRadius(const double &start_t, const double &des_radius,
    ...
    while (radius < des_radius &&
      ...
      seg_time += delt;
      seg_time =
          std::min(seg_time, global_duration_ - start_t); // 使用std::min代替min
...
struct LocalTrajData {
  ...
  int traj_id_;
  double duration_;
  rclcpp::Time start_time_; // ROS2时间类型
