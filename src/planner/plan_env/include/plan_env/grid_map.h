...
struct MappingParameters {
  ...
  double visualization_truncate_height_, virtual_ceil_height_, ground_height_,
      virtual_ceil_yp_, virtual_ceil_yn_;
  bool show_occ_time_;
...
private:
  ...
  // 修改：回调函数参数类型改为ROS2消息的共享指针
  void depthPoseCallback(const Image::SharedPtr img,
                         const PoseStamped::SharedPtr pose);
  void extrinsicCallback(const Odometry::SharedPtr odom);
  void depthOdomCallback(const Image::SharedPtr img,
                         const Odometry::SharedPtr odom);
  void cloudCallback(const PointCloud2::SharedPtr img);
  ...
  // 修改：消息同步策略使用ROS2类型
  typedef message_filters::sync_policies::ApproximateTime<Image, Odometry>
      SyncPolicyImageOdom;
  typedef message_filters::sync_policies::ApproximateTime<Image, PoseStamped>
      SyncPolicyImagePose;
  ...
  // 修改：使用rclcpp::Node的共享指针
  rclcpp::Node::SharedPtr node_;
  // 修改：ROS2订阅器类型
  shared_ptr<message_filters::Subscriber<Image>> depth_sub_;
  shared_ptr<message_filters::Subscriber<PoseStamped>> pose_sub_;
  shared_ptr<message_filters::Subscriber<Odometry>> odom_sub_;
  SynchronizerImagePose sync_image_pose_;
  SynchronizerImageOdom sync_image_odom_;

  // 修改：ROS2订阅器和发布器类型
  rclcpp::Subscription<PointCloud2>::SharedPtr indep_cloud_sub_;
  rclcpp::Subscription<Odometry>::SharedPtr indep_odom_sub_;
  rclcpp::Subscription<Odometry>::SharedPtr extrinsic_sub_;
  rclcpp::Publisher<nav_msgs::msg::OccupancyGrid>::SharedPtr map_pub_;
  rclcpp::Publisher<nav_msgs::msg::OccupancyGrid>::SharedPtr map_inf_pub_;
  // 修改：ROS2定时器类型
