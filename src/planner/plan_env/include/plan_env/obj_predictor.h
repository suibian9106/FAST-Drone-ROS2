...
public:
  ...
  Eigen::Vector3d evaluateConstVel(double t) {
    ...
    // 修改：使用rclcpp::Time的seconds()方法
    tv << 1.0, pow(t - global_start_time_.seconds(), 1);

    // cout << t - global_start_time_.seconds() << endl;
...
public:
  ...
  // 修改：参数类型改为rclcpp::Time
  void init(int id, int skip_num, int queue_size,
            rclcpp::Time global_start_time);
...
private:
  ...
  // 修改：ROS2订阅器类型
  vector<rclcpp::Subscription<PoseStamped>::SharedPtr> pose_subs_;
  ...
  // 修改：回调函数参数类型改为ROS2消息的共享指针
  void markerCallback(const Marker::SharedPtr msg);
