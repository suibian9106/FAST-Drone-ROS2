...
void RC_Data_t::check_validity() {
    if (mode >= -1.1 && mode <= 1.1 && gear >= -1.1 && gear <= 1.1 &&
        ...
        RCLCPP_ERROR(rclcpp::get_logger("rc_data"),
            "RC data validity check fail. mode=%f, gear=%f, reboot_cmd=%f",
            mode, gear, reboot_cmd);
    }
}

bool RC_Data_t::check_centered() {
    bool centered = abs(ch[0]) < 1e-5 && abs(ch[0]) < 1e-5 &&
                    abs(ch[0]) < 1e-5 && abs(ch[0]) < 1e-5;
    return centered;
}
...
// #define VEL_IN_BODY
    ...
    static int count = 0;
    if (count++ % 500 == 0) RCLCPP_WARN(rclcpp::get_logger("odom_data"), "VEL_IN_BODY!!!");
#endif

    // check the frequency
    static int one_min_count = 9999;
    static rclcpp::Time last_clear_count_time = rclcpp::Time(0, 0);
    if ((now - last_clear_count_time).seconds() > 1.0) {
        if (one_min_count < 100) {
            RCLCPP_WARN(rclcpp::get_logger("odom_data"),
...
Imu_Data_t::Imu_Data_t() { rcv_stamp = rclcpp::Time(0); }
...
void Imu_Data_t::feed(const sensor_msgs::msg::Imu::SharedPtr pMsg) {
    ...
    if ((now - last_clear_count_time).seconds() > 1.0) {
        if (one_min_count < 100) {
            RCLCPP_WARN(rclcpp::get_logger("imu_data"),
...
Command_Data_t::Command_Data_t() { rcv_stamp = rclcpp::Time(0); }
...
Battery_Data_t::Battery_Data_t() { rcv_stamp = rclcpp::Time(0); }
...
void Battery_Data_t::feed(const sensor_msgs::msg::BatteryState::SharedPtr pMsg) {
    ...
    static rclcpp::Time last_print_t = rclcpp::Time(0, 0);
    if (percentage > 0.05) {
        if ((rcv_stamp - last_print_t).seconds() > 10) {
            RCLCPP_INFO(rclcpp::get_logger("battery_data"),
                "[px4ctrl] Voltage=%.3f, percentage=%.3f", volt,
                percentage);
            last_print_t = rcv_stamp;
        }
    } else {
        if ((rcv_stamp - last_print_t).seconds() > 1) {
            // RCLCPP_ERROR("[px4ctrl] Dangerous! voltage=%.3f, percentage=%.3f",
            // volt, percentage);
            last_print_t = rcv_stamp;
        }
    }
...
Takeoff_Land_Data_t::Takeoff_Land_Data_t() { rcv_stamp = rclcpp::Time(0); }
