...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<rclcpp::Node>("example_control_node");
    ...
    // 让无人机转向90度
    while (rclcpp::ok() && control_interface.isStartPositionSet()) {
        control_interface.sendTargetAngleOffsetLast(90.0);  // 转向90度
        // 等待无人机完成转向
        double offset = control_interface.reportCurrentAngle();
        // 使用角度阈值
        if (std::abs(offset - 90.0) < 5.0) {
            RCLCPP_INFO(node->get_logger(), "Reached target angle offset: %f", offset);
            break;  // 退出循环，转向完成
        }
        rclcpp::spin_some(node);
        rate.sleep();
    ...
    // 让无人机转向为初始时的朝向
    while (rclcpp::ok() && control_interface.isStartPositionSet()) {
        Eigen::Vector3d start_angles = control_interface.getStartAngles();
        control_interface.sendTargetAngleOffsetStart(start_angles.z() * 180.0 /
                                                     M_PI);  // 转向初始朝向
        // 等待无人机完成转向
        double offset = control_interface.reportAngleOffsetStart();
        // 使用角度阈值
